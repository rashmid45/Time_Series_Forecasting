...
sf_PNew = sf_punjab.groupby('ACTUALDATE',as_index = False).sum()
sf_PNew.head()
...
#sorting the data
sf_p = sf_PNew.sort_values('ACTUALDATE')
sf_p.head()
sf_p.tail()

sf_p.to_csv('Punjab.csv')

sf_p.plot(color='maroon',title='Punjab',figsize= (15,6))
...
autocorrelation_plot(sf_p,color='maroon')

# time series decomposition
c= seasonal_decompose(sf_p,period=1 , model='mul')
...
stepwise_model = auto_arima(sf_p, start_p=1, start_q=1,
...
sf_p.info()
sf_p.head()
train = sf_p.loc['2014-01-01':'2016-12-01']
len(train) # 36


test = sf_p.loc['2017-01-01':]
len(test)
